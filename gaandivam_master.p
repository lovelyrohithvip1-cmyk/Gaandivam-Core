...
def run_agent(script_name):
    ...
    try:
        ...
        # Fallback to 'python' just in case 'py' isn't recognized by subprocess
        try:
            subprocess.run(["python", script_name], check=True)
        except Exception as e:
            print(f"❌ ERROR: Could not find Python executable to run {script_name}.")
    time.sleep(1) # Brief pause to ensure files are fully written to disk
...
def master_run():
    ...
    # PHASE 3: Gamification & Assessment
